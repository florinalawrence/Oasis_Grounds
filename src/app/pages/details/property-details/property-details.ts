...
export class PropertyDetails implements OnInit {
        ...
        if (id) {
          ...
          this.route.queryParamMap
            ...
            .subscribe(queryParams => {
              ...
              if (queryId) {
                console.log('🆔 Property ID from query params:', queryId);
    ...
    this.propertyService.getPropertyById(propertyId)
      ...
      .subscribe({
        next: (response: any) => {
          ...
          if (propertyData) {
            console.log('✅ Property data loaded successfully:', propertyData.title);
            this.propertyData.set(propertyData);
            this.loading.set(false);
            this.loader.hide();
          } else {
            console.error('❌ Property data not found in response');
            this.error.set('Property data not found in response');
            this.loading.set(false);
            this.loader.hide();
          }
  ...
  retry(): void {
    const id = this.propertyId();
    if (id) {
      console.log('🔄 Retrying to load property:', id);
      this.loadPropertyDetails(id);
    } else {
      console.error('❌ Cannot retry: No property ID available');
